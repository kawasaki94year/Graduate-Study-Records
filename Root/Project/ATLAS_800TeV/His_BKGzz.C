...
Eventselection3::Eventselection3(){
}

void Eventselection3::Draw_BKGzz(){
  TChain *BKG_mu_tree = new TChain("Delphes");//tree name
  BKG_mu_tree->Add("/home/xwl/Analysis/Root/pp/background/zz_100TeV_800.root");
	   //muon
  ...
  BKG_mu_tree->SetBranchAddress("Jet.Eta", Jet_Eta, &b_Jet_Eta);
           //output root file
  //TFile *f = new TFile("/home/xwl/Analysis/pp-100TeV/ATLAS_800TeV/Root/bkgzz1_800.root","recreate");
  TFile *f = new TFile("/home/xwl/Analysis/pp-100TeV/ATLAS_800TeV/Root/bkgzz1_800_cut.root","recreate");
           //creat histogram
  ...
  for (Long64_t i = 0; i < neventsBKG; i++){
	...
	if(num_e == 1 && num_mu == 1 && Jet_size == 0){
	  if( Electron_PT[0] > 100 && Muon_PT[0] > 100 && l_MET > 300){
        ...
        cout<<"zz:num_cut = "<<num_cut<<";"<<"eff_cut = " << eff_cut << endl;
        
        cout<<"zz:num_cut1 for ee="<<num_cut1<<endl;
	cout<<"zz:eff_cut1 for ee="<<eff_cut1<<endl;
	
	cout<<"zz:num_cut2 for mu mu="<<num_cut2<<endl;
	cout<<"zz:eff_cut2 for mu mu="<<eff_cut2<<endl;
	
	cout<<"zz:num_cut3 for emu="<<num_cut3<<endl;
	cout<<"zz:eff_cut3 for emu="<<eff_cut3<<endl;
      }
    }
  }
  f->Write();
