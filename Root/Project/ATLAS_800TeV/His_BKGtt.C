...
Eventselection1::Eventselection1(){
}

void Eventselection1::Draw_BKGtt(){
  TChain *BKG_mu_tree = new TChain("Delphes");//tree name
  BKG_mu_tree->Add("/home/xwl/Analysis/Root/pp/background/tt_100TeV_800.root");
	   //muon
  BKG_mu_tree->SetBranchAddress("Muon_size",&Muon_size, &b_Muon_size);
  BKG_mu_tree->SetBranchAddress("Muon.Eta", Muon_Eta, &b_Muon_Eta);
  BKG_mu_tree->SetBranchAddress("Muon.PT", Muon_PT, &b_Muon_PT);
  BKG_mu_tree->SetBranchAddress("Muon.Charge", Muon_Charge, &b_Muon_Charge);
  BKG_mu_tree->SetBranchAddress("Muon.Phi", Muon_Phi, &b_Muon_Phi);
           //MissingET
  BKG_mu_tree->SetBranchAddress("MissingET_size",&MissingET_size, &b_MissingET_size);
  BKG_mu_tree->SetBranchAddress("MissingET.MET", MissingET_MET, &b_MissingET_MET);
	   //Electron
  BKG_mu_tree->SetBranchAddress("Electron_size",&Electron_size, &b_Electron_size);
  BKG_mu_tree->SetBranchAddress("Electron.Charge", Electron_Charge, &b_Electron_Charge);
  BKG_mu_tree->SetBranchAddress("Electron.Eta", Electron_Eta, &b_Electron_Eta);
  BKG_mu_tree->SetBranchAddress("Electron.PT", Electron_PT, &b_Electron_PT);
  BKG_mu_tree->SetBranchAddress("Electron.Phi", Electron_Phi, &b_Electron_Phi);
	   //Jet
  BKG_mu_tree->SetBranchAddress("Jet_size",&Jet_size, &b_Jet_size);
  BKG_mu_tree->SetBranchAddress("Jet.PT", Jet_PT, &b_Jet_PT);
  BKG_mu_tree->SetBranchAddress("Jet.Eta", Jet_Eta, &b_Jet_Eta);
           //output root file
  //TFile *f = new TFile("/home/xwl/Analysis/pp-100TeV/ATLAS_800TeV/Root/bkgtt1_800.root","recreate");
  TFile *f = new TFile("/home/xwl/Analysis/pp-100TeV/ATLAS_800TeV/Root/bkgtt1_800_cut.root","recreate");
           //creat histogram
  ...
  double num_cut3 = 0;
  double eff_cut3 = 0;
  Long64_t neventsBKG = BKG_mu_tree->GetEntries();
  cout<<"entries = "<<neventsBKG<<endl;
  for (Long64_t i = 0; i < neventsBKG; i++){
    BKG_mu_tree->GetEntry(i);
         //muon
    int num_mu = 0;
	...
	if(num_e == 2 && Jet_size == 0){
          if(Electron_PT[0] > 100 && Electron_PT[1] > 100 && l_MET > 300){
              //four momentum
	    ...
	    double mVisA = 0; // mass of visible object on side A.  Must be >=0.
	      ...
	      num_cut1 = num_cut1 + 1;
	      eff_cut1 = num_cut1 / neventsBKG;
              cout<<"entries1111 = "<<neventsBKG<<endl;
	    }
	  }
	}
	...
	if(num_mu == 2 && Jet_size == 0){
	  if( Muon_PT[0] > 100 && Muon_PT[1] > 100 && l_MET > 300){
		  //four momentum
	    l_PT[0] = Muon_PT[0];
	    l_PT[1] = Muon_PT[1];
	    l_Eta[0] = Muon_Eta[0];
	    ...
	    if(ll_M > 130 && MTmumu > 100){
	      ...
	      num_cut2 = num_cut2 + 1;
	      eff_cut2 = num_cut2 / neventsBKG;
              cout<<"entries2222 = "<<neventsBKG<<endl;
	    }
	  }
	}
	...
	if(num_e == 1 && num_mu == 1 && Jet_size == 0){
	  if(Electron_PT[0] > 100 &&  Muon_PT[0] > 100 && l_MET > 300){
		    //four momentum
	    l_PT[0] = Electron_PT[0];
	    l_PT[1] = Muon_PT[0];
	    l_Eta[0] = Electron_Eta[0];
	    ...
	    if(ll_M > 130 && MTemu > 100){
	      ...
	      num_cut3 = num_cut3 + 1;
	      eff_cut3 = num_cut3 / neventsBKG;
              cout<<"entries3333 = "<<neventsBKG<<endl;
	    }
	  }
	}
	...
	cout<<"tt:num_cut = "<<num_cut<<";"<<"eff_cut = " << eff_cut<<endl;
	
	cout<<"tt:num_cut1 for ee="<<num_cut1<<endl;
	cout<<"tt:eff_cut1 for ee="<<eff_cut1<<endl;
	
	cout<<"tt:num_cut2 for mu mu="<<num_cut2<<endl;
	cout<<"tt:eff_cut2 for mu mu="<<eff_cut2<<endl;
	
	cout<<"tt:num_cut3 for emu="<<num_cut3<<endl;
	cout<<"tt:eff_cut3 for emu="<<eff_cut3<<endl;
      }
    }
  }
  f->Write();
